...
async function testApiCreateLine() {
  ...
  const lReq = http.request({
    ...
    const oReq = http.request({
      ...
      let oraw = ''; ores.on('data', c => oraw += c); ores.on('end', () => {
        ...
        // 3. /api/order-line/create
        // We will try passing price_unit and name, product_id 27124 (some product)
        const qReq = http.request({
          hostname: 'localhost', port: 3001, 
          path: `/proxy/api/order-line/create?order_id=${oid}&product_id=27124&price_unit=-10.0&name=DiscountTest`, 
          method: 'GET',
          headers: { 'Origin': 'http://localhost', 'Cookie': cookie }
        }, qres => {
